...
class DataManager:
    ...
    def import_json(self, filepath: str) ->dict:
        try:
            ...
            # --- Bước 1: parse toàn bộ dữ liệu ra biến TẠM trước.
            # Nếu có lỗi ở đây (field thiếu, kiểu dữ liệu sai...), exception sẽ
            # nhảy vào except bên dưới TRƯỚC KHI bất kỳ state nào của self._um
            # bị thay đổi -> dữ liệu cũ được giữ nguyên vẹn khi import thất bại.
            new_users = []
            new_id_map = {}
            max_id = 0
            ...
            # --- Bước 2: mọi thứ parse xong thành công -> giờ mới ghi đè state thật.
            # Reset cả _pending/_blocked ở đây: nếu không, request/block cũ (gắn với
            # user_id cũ) sẽ vô tình áp dụng nhầm lên user_id mới sau khi import
            # (vì user_id được sinh lại từ đầu U001, U002... rất dễ trùng ID cũ).
    ...
    def import_csv(self,
        ...
        try:
            # --- Bước 1: đọc + parse toàn bộ 2 file CSV ra biến TẠM trước.
            # Nếu 1 dòng nào lỗi (vd age không phải số), exception nhảy vào
            # except bên dưới TRƯỚC KHI state thật của self._um bị đụng tới.
            new_users = []
            new_id_map = {}
            max_id = 0
            with open(users_filepath,
            ...
            # --- Bước 2: parse xong toàn bộ, không lỗi -> mới ghi đè state thật.
            self._um._users = new_users
            self._um._id_map = new_id_map
            self._um._graph = new_graph
            self._um._avl_name = new_avl
